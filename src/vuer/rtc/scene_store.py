...
def find_by_key(node: Union[SceneNode, SceneState], key: str) -> Optional[SceneNode]:
  """
...
def update_node(state: SceneState, key: str, props: Dict[str, Any]) -> SceneState:
  """
  ...
  for k, v in props.items():
    if k == "children":
      ...
      if isinstance(v, list):
        node.children = [
          SceneNode.from_dict(c) if isinstance(c, dict) else c for c in v
        ]
    elif k == "tag":
      node.tag = v
    else:
...
class SceneStore:
  ...
  async def set_scene(
    ...
    async with self._lock:
      self._state = SceneState(
        children=[self._to_node(c) for c in (children or [])],
        bgChildren=[self._to_node(c) for c in (bgChildren or [])],
        htmlChildren=[self._to_node(c) for c in (htmlChildren or [])],
        rawChildren=[self._to_node(c) for c in (rawChildren or [])],
  ...
  def _upsert_sync(self, nodes: Union["Element", List["Element"]]) -> "asyncio.Task":
  ...
  def _remove_sync(self, keys: Union[str, List[str]]) -> "asyncio.Task":

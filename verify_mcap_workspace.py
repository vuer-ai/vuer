...
What is verified
----------------
1.  Construction  — workspace_from_config auto-composes layers
2.  Index stats   — attachment count, channel count, total messages
3.  Memory model  — confirms NO bytes in RAM (only refs)
4.  Overlay paths — ws.paths contains only filesystem dirs (no .mcap)
5.  MCAP file list — mcap_ws.mcap_files
6.  Attachment resolve — await ws.resolve(name) → Blob
7.  Topic link registration — each channel has a URL in mcap_ws.links
...
async def verify(mcap_path: Path, extra_dirs: list[Path]) -> None:
    ...
    ws = workspace_from_config(all_paths)
    elapsed = time.perf_counter() - t0

    # McapWorkspace for MCAP-only checks (attachments, channels, topic links, etc.)
    mcap_ws = McapWorkspace(str(mcap_path))

    if extra_dirs:
        if not isinstance(ws, OverlayWorkspace):
            fail(f"workspace_from_config with extra dirs should return OverlayWorkspace, got {type(ws).__name__}")
        mcap_layer = next((l for l in ws._layers if isinstance(l, McapWorkspace)), None)
        fs_layer   = next((l for l in ws._layers if isinstance(l, FilesystemWorkspace)), None)
        if mcap_layer is None:
            fail("OverlayWorkspace missing McapWorkspace layer")
        if fs_layer is None:
            fail("OverlayWorkspace missing FilesystemWorkspace layer")
        ok(f"workspace_from_config → OverlayWorkspace(McapWorkspace, FilesystemWorkspace) in {elapsed*1000:.1f} ms")
    else:
        if not isinstance(ws, McapWorkspace):
            fail(f"workspace_from_config('.mcap') should return McapWorkspace, got {type(ws).__name__}")
        ok(f"workspace_from_config → McapWorkspace in {elapsed*1000:.1f} ms")

    ok("McapWorkspace(*mcap_path) explicit construction OK")

    # ── 2. Index stats ────────────────────────────────────────────────────────
    section("2. Index stats")

    n_attachments = len(mcap_ws.attachments)
    n_channels    = len(mcap_ws.channels)
    total_msgs    = sum(len(ch.log_times) for ch in mcap_ws.channels.values())
    ...
    for ref in mcap_ws.attachments.values():
    ...
    for ch in mcap_ws.channels.values():
    ...
    if mcap_path not in mcap_ws.mcap_files:
        fail(f"{mcap_path} not in mcap_ws.mcap_files: {mcap_ws.mcap_files}")
    ok(f"mcap_ws.mcap_files = {mcap_ws.mcap_files}")
    ...
    if n_attachments == 0:
        ...
        for name in list(mcap_ws.attachments.keys())[:3]:  # check up to 3
    ...
    # 6b. Non-existent attachment → None (or filesystem Path, not a Blob)
    ...
    if n_channels == 0:
        ...
        registered_links = set(mcap_ws.links.keys())
        missing = []
        for topic in list(mcap_ws.channels.keys())[:5]:  # spot-check up to 5
    ...
    if n_channels == 0:
        ...
        topic = next(iter(mcap_ws.channels))
        url   = "/topics/" + topic.lstrip("/")

        resp = await _call_link(mcap_ws, url, query={})
        if resp is None:
            fail(f"GET {url} returned None")
        if not isinstance(resp, web.Response):
            fail(f"GET {url} returned {type(resp)}, expected web.Response")
        ch = mcap_ws.channels[topic]
        ok(
    ...
    if n_channels == 0:
        ...
        resp = await _call_link(mcap_ws, url, query={"t": t_str})
        if resp is None:
        ...
        t_ns_str = str(t_ns)
        resp2 = await _call_link(mcap_ws, url, query={"t": t_ns_str})
        if resp2 is None:
    ...
    all_topics = mcap_ws.glob("*", wd="topics/")
    ...
    all_atts = mcap_ws.glob("*", wd="attachments/")
    ...
    if all_topics:
        ...
        matched = mcap_ws.glob(f"{prefix}*", wd="topics/")
    ...
    records_h = mcap_ws.head(n=n_head)
    ...
    tree_root = mcap_ws.tree(level=2)
    ...
    for topic, ch in mcap_ws.channels.items():
    ...
    if "json" in BUILTIN_DECODERS:
        ...
        ok("Built-in json decoder still works alongside custom decoder")
    ...
    print(f"\n  Workspace summary:")
    print(f"    Composite   : {ws}")
    print(f"    MCAP files  : {mcap_ws.mcap_files}")
    print(f"    Attachments : {n_attachments}")
    print(f"    Channels    : {n_channels}")
    print(f"    Messages    : {total_msgs}")
    print(f"    Links       : {len(mcap_ws.links)}")
    print()
